...
namespace experiment
{
    namespace nonhop
    {
        ...
        {
        public:
            void operator()(graph<weight_type> &input_graph, nonhop::two_hop_case_info<hop_weight_type> &case_info) const;
        ...
        private:
            ...
            void clean_L(nonhop::two_hop_case_info<hop_weight_type> &case_info, int thread_num) const;

            void PLL_clear_global_values() const;
        };

        template <typename weight_type, typename hop_weight_type>
        inline void GeneratorNonHop<weight_type, hop_weight_type>::operator()(graph<weight_type> &input_graph, nonhop::two_hop_case_info<hop_weight_type> &case_info) const
        {
            ...
            generator_timer.startSubtask("step 2: generate labels");
            {
                ...
                for (int v_k = 0; v_k <= last_check_vID; v_k++)
                {
                    results.emplace_back(
                        pool.enqueue([this, v_k, &input_graph, last_check_vID] { // pass const type value j to thread; [] can be empty
        ...
        template <typename weight_type, typename hop_weight_type>
        inline void GeneratorNonHop<weight_type, hop_weight_type>::PLL_dij_function(int v_k, graph<weight_type> &input_graph) const
        {
        ...
        template <typename weight_type, typename hop_weight_type>
        inline std::vector<std::vector<two_hop_label<hop_weight_type>>> GeneratorNonHop<weight_type, hop_weight_type>::sortL(int num_of_threads) const
        {
            ...
            for (int v_k = 0; v_k < N; v_k++)
            {
                results.emplace_back(
                    pool.enqueue([this, &output_L, v_k] { // pass const type value j to thread; [] can be empty
                        std::sort(L_temp_595<hop_weight_type>[v_k].begin(), L_temp_595<hop_weight_type>[v_k].end(), experiment::nonhop::compare_two_hop_label_small_to_large<hop_weight_type>);
                        std::vector<two_hop_label<hop_weight_type>>(L_temp_595<hop_weight_type>[v_k]).swap(L_temp_595<hop_weight_type>[v_k]);
                        output_L[v_k] = L_temp_595<hop_weight_type>[v_k];
                        std::vector<two_hop_label<hop_weight_type>>().swap(L_temp_595<hop_weight_type>[v_k]); // clear new labels for RAM efficiency
        ...
        template <typename weight_type, typename hop_weight_type>
        inline void GeneratorNonHop<weight_type, hop_weight_type>::clean_L(nonhop::two_hop_case_info<hop_weight_type> &case_info, int thread_num) const
        {
        ...
        template <typename weight_type, typename hop_weight_type>
        inline void GeneratorNonHop<weight_type, hop_weight_type>::PLL_clear_global_values() const
        {
