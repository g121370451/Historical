...
namespace experiment {
    ...
    namespace nonhop {
        ...
        std::pair<hop_weight_type, int> graph_weighted_two_hop_extract_distance_and_hub_in_current_with_csv(
            ...
            hop_weight_type distance = std::numeric_limits<hop_weight_type>::max();
            // if disconnected, return this large value
            int common_hub = -1;
                ...
                for (auto vector1_begin = vector1_check_pointer; vector1_begin != pointer_L_s_end; ++vector1_begin) {
                    ...
                    for (auto vector2_begin = vector2_check_pointer;
                        ...
                        if (vector1_begin->vertex == vector2_begin->vertex &&
                            ...
                            if (distance > dis) {
                                distance = dis;
//                                 std::cout << "x (" << vector1_begin->vertex << "," << vector1_begin->distance <<  ") " << std::endl;
//                                 std::cout << "y (" << vector2_begin->vertex << "," << vector2_begin->distance <<  ") " << std::endl;
                            }
                        }
                    }
                }
...
//                                std::cout << "y (" << vector2_begin->vertex << ","  << vector2_begin->distance << "," <<vector2_begin->t_s <<"," << vector2_begin->t_e <<  ") " << std::endl;
    ...
    namespace hop {
        ...
        graph_weighted_two_hop_extract_all_distances_under_hop_limit(
            ...
            using result_tuple = std::tuple<hop_weight_type, int, int>;
            const hop_weight_type INF = std::numeric_limits<hop_weight_type>::max();
            constexpr int INF_HOP = std::numeric_limits<int>::max();

            std::vector<result_tuple> result(hop_cst + 1, {INF, INF_HOP, -1});

            if (source == terminal) {
                for (int h = 0; h <= hop_cst; ++h) {
                    result[h] = {0, 0, source};
                }
                return result;
            }
            ...
            while (vector1_check_pointer != pointer_L_s_end && vector2_check_pointer != pointer_L_t_end) {
                if (vector1_check_pointer->hub_vertex == vector2_check_pointer->hub_vertex) {
                    ...
                    for (auto vector1_begin = vector1_check_pointer; vector1_begin != vector1_end; ++vector1_begin) {
                        for (auto vector2_begin = vector2_check_pointer;
                             vector2_begin != vector2_end; ++vector2_begin) {
                            int total_hop = vector1_begin->hop + vector2_begin->hop;
                            if (total_hop > hop_cst)
                                break;
                            ...
                            // 更新所有 hop 限制下的最优解
                            for (int h = total_hop; h <= hop_cst; ++h) {
                                if (std::get<0>(result[h]) > dis) {
                                    result[h] = {dis, total_hop, vector1_begin->hub_vertex};
                                }
                            }
                        }
                    }
