                    ...
                    for (auto nei: instance_graph[it.first]) {
                        if (it.second < nei.first) {
                            ...
                            if (d_old <= search_weight && search_weight < std::numeric_limits<hop_weight_type>::max()) {
                                ...
                                this->list_infinite.emplace_back(nei.first, it.second, it.hop + 1,
                                 std::numeric_limits<hop_weight_type>::max(), it.dis + nei.second);
                                mtx_599_1.unlock();
                            }
                        }
                    }
                        ...
                        for (int hop_i = 1; hop_i <= hop_vn; hop_i++) {
                            ...
                            if (query_dis > di) {
                                ...
                                insert_sorted_hop_constrained_two_hop_label_with_csv(
                                    ...
                                    shard);
                                this->list.emplace_back(diffuseVertex, targetVertex, hop_i, di, query_dis);
                                L_lock[diffuseVertex].unlock();
                                mtx_599_1.lock();
