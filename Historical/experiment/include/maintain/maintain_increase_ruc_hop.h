    ...
    void Strategy2024HopIncrease<weight_type, hop_weight_type>::HOP_maintain_SPREAD2_batch(
        ...
        for (const auto &it: al2) {
            results_dynamic.emplace_back(pool_dynamic.enqueue(
                [&it, L, PPR, al3, &instance_graph, upper_k,&cost0,&cost1, &cost2, &cost3, &cost4,&cost5]() {
                    mtx_599_1.lock();
                    const int current_tid = Qid_599.front();
                    Qid_599.pop();
                    mtx_599_1.unlock();
                    ...
                    double cost14 = 0;
                    double cost15 = 0;
                    auto time00 = std::chrono::high_resolution_clock::now();
                    ppr_lock[v].lock();
                    std::vector<int> temp = PPR_TYPE::PPR_retrieve(*PPR, v, u);
                    ppr_lock[v].unlock();
                    temp.push_back(u);
                    auto time01 = std::chrono::high_resolution_clock::now();
                    cost10 += std::chrono::duration_cast<std::chrono::duration<double> >(time01 - time00).count();
                    for (auto t: temp) {
                        ...
                        for (int hop_i = 1; hop_i <= hop_vn; hop_i++) {
                            ...
                            if (query_dis > di) {
                                ...
                                cost15 += std::chrono::duration_cast<std::chrono::duration<double> >(time10 - time9).
                                        count();
                            }
                        }
                    ...
                    // std::cout << "spread inner " << cost10 <<" " << cost11 << " " << cost12 << " " << cost13 << " " << cost14 << " " << cost15 << std::endl;
                    mtx_599_1.lock();
                    Qid_599.push(current_tid);
                    mtx_599_1.unlock();
                    return 1;
        ...
        std::cout << "Spread2 "<< r0 << " " << r1 <<" "<< r2 << " "<< r3 << " " << r4 << " " << r5  << std::endl;
        std::vector<std::future<int> >().swap(results_dynamic);
    }
