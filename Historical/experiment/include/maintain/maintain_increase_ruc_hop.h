        ...
        for (auto &al3_item: al3_map) {
            results_dynamic.emplace_back(pool_dynamic.enqueue([time, al3_item, L, &instance_graph, PPR, upper_k] {
                ...
                while (!pq.empty()) {
                    ...
                    L_lock[x].lock();
                    std::pair<int, int> d_old = search_sorted_two_hop_label_in_current_with_less_than_k_limit_with_csv((*L)[x], v, xhv,shard);
                    L_lock[x].unlock();
                ...
                for (auto t: temp) {
                    if (v < t) {
                        ...
                        for (const auto &nei: instance_graph[t]) {
                            ...
                            std::pair<hop_weight_type, int> dis_hop = search_sorted_two_hop_label_in_current_with_csv((*L)[nei.first], v,shard);
                    ...
                    if (t < v) {
                        ...
                        for (const auto &nei: instance_graph[v]) {
                            std::pair<hop_weight_type, int> dis_hop = search_sorted_two_hop_label_in_current_with_csv((*L)[nei.first], t,shard);
                            if (d1 > dis_hop.first + nei.second) {
                                d1 = dis_hop.first + nei.second;
                                hop_vn = dis_hop.second;
                            }
