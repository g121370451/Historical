    ...
    void StrategyA2021HopDecrease<weight_type, hop_weight_type>::ProDecreasep_batch(graph<weight_type> &instance_graph,
        ...
        for (const auto &it: CL_curr) {
            results_dynamic.emplace_back(
                    pool_dynamic.enqueue([time, it, L, PPR, CL_next, &instance_graph, upper_k, this] {
                        mtx_599_1.lock();
                        ...
                        for (auto nei: instance_graph[v]) {
                            ...
                            if (u < vnei) {
                                ...
                                if (query_dis > dnew) {
                                    L_lock[vnei].lock();
                                    insert_sorted_hop_constrained_two_hop_label_with_csv((*L)[vnei], u, hop_u + 1, dnew,
                                                                                         time,
                                                                                         shard);
                                    L_lock[vnei].unlock();
                                    mtx_list_check.lock();
                                    ...
                                    if (label_dis < std::numeric_limits<hop_weight_type>::max() && label_dis > dnew) {
                                        L_lock[vnei].lock();
                                        insert_sorted_hop_constrained_two_hop_label_with_csv((*L)[vnei], u, hop_u + 1,
                                                                                             dnew,
                                                                                             time,
                                                                                             shard);
                                        L_lock[vnei].unlock();
