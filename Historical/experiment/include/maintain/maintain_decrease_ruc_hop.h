...
namespace experiment::hop::ruc::decrease {
    ...
    template<typename weight_type, typename hop_weight_type>
    void Strategy2024NonHopDecrease<weight_type, hop_weight_type>::operator()(graph<weight_type> &instance_graph,
        ...
        std::vector<hop_constrained_affected_label<hop_weight_type> > CL;
        decrease_maintain_step1_batch(w_new_map, &mm.L, &mm.PPR, &CL, pool_dynamic, results_dynamic, time, mm.upper_k);
                ...
                for (int sl = 0; sl < 2; sl++) {
                    ...
                    for (auto &it: (*L)[v1]) {
                        if (it.hub_vertex <= v2 && it.hop + 1 < upper_k && static_cast<long long int>(it.distance) +
                            w_new <
                            TwoM_value && it.t_e == std::numeric_limits<int>::max()) {
    ...
    template<typename weight_type, typename hop_weight_type>
    void Strategy2024NonHopDecrease<weight_type, hop_weight_type>::DIFFUSE_batch(graph<weight_type> &instance_graph,
        std::vector<std::vector<two_hop_label<hop_weight_type> > > *L, PPR_TYPE::PPR_type *PPR,
        std::vector<hop_constrained_affected_label<hop_weight_type> > &CL, ThreadPool &pool_dynamic,
