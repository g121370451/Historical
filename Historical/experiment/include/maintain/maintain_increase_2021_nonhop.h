	...
	namespace nonhop
	{
		namespace algorithm2021
		{
			namespace increase
			{
				...
				{
				public:
					void operator()(graph<weight_type> &instance_graph, two_hop_case_info<hop_weight_type> &mm, std::vector<std::pair<int, int>> &v, std::vector<weight_type> &w_old_vec,
									ThreadPool &pool_dynamic, std::vector<std::future<int>> &results_dynamic, int time) const;

				private:
					void PI11(graph<weight_type> &instance_graph, std::vector<std::vector<two_hop_label<hop_weight_type>>> *L,
					...
					void PI12(graph<weight_type> &instance_graph, std::vector<std::vector<two_hop_label<hop_weight_type>>> *L, PPR_TYPE::PPR_type *PPR,
					...
					void PI22(graph<weight_type> &instance_graph, std::vector<std::vector<two_hop_label<hop_weight_type>>> *L, PPR_TYPE::PPR_type *PPR,
				...
				template <typename weight_type, typename hop_weight_type>
				inline void StrategyA2021NonHopIncrease<weight_type, hop_weight_type>::operator()(graph<weight_type> &instance_graph, two_hop_case_info<hop_weight_type> &mm, std::vector<std::pair<int, int>> &v, std::vector<weight_type> &w_old_vec, ThreadPool &pool_dynamic, std::vector<std::future<int>> &results_dynamic, int time) const
				{
					...
					for (auto &iter : w_old_map)
					{
						...
						{
							mtx_595[v2].lock();
							auto search_weight = search_sorted_two_hop_label_in_current_with_csv(mm.L[v2], it.vertex, shard);
							mtx_595[v2].unlock();
							if (it.vertex <= v2 && search_weight.first >= it.distance + w_old && search_weight.first < std::numeric_limits<int>::max())
							{
						...
						{
							mtx_595[v1].lock();
							auto search_weight = search_sorted_two_hop_label_in_current_with_csv(mm.L[v1], it.vertex, shard);
							mtx_595[v1].unlock();
							if (it.vertex <= v1 && search_weight.first >= it.distance + w_old && search_weight.first < std::numeric_limits<int>::max())
							{
				...
				template <typename weight_type, typename hop_weight_type>
				inline void StrategyA2021NonHopIncrease<weight_type, hop_weight_type>::PI11(graph<weight_type> &instance_graph, std::vector<std::vector<two_hop_label<hop_weight_type>>> *L, std::vector<affected_label> &al1_curr, std::vector<affected_label> *al1_next, std::map<std::pair<int, int>, weight_type> &w_old_map, ThreadPool &pool_dynamic, std::vector<std::future<int>> &results_dynamic, int time)
				{
					bool is_debug = false;
					...
					for (auto it : al1_curr)
					{
						results_dynamic.emplace_back(pool_dynamic.enqueue([it, L, al1_next, &instance_graph, &w_old_map, time, is_debug]
							...
							for (auto nei : instance_graph[it.first]) {
								...
								std::pair<int,int> search_weight = search_sorted_two_hop_label_in_current_with_csv((*L)[nei.first], it.second,shard);
								...
								if (it.dis + w_old == search_weight.first) {
									mtx_595_1.lock();
									...
									// }
									al1_next->push_back(affected_label(nei.first, it.second, search_weight.first));
									mtx_595_1.unlock();
								}
							}
							mtx_595[it.first].lock();
							insert_sorted_two_hop_label((*L)[it.first], it.second, MAX_VALUE, time, shard); // this does not change the size of L[it->first] here, so does not need to lock here
				...
				template <typename weight_type, typename hop_weight_type>
				inline void StrategyA2021NonHopIncrease<weight_type, hop_weight_type>::PI12(graph<weight_type> &instance_graph, std::vector<std::vector<two_hop_label<hop_weight_type>>> *L, PPR_TYPE::PPR_type *PPR, std::vector<affected_label> &al1_curr, std::vector<pair_label> *al2_next, ThreadPool &pool_dynamic, std::vector<std::future<int>> &results_dynamic, int time)
				{
					for (auto it : al1_curr)
					{
						results_dynamic.emplace_back(pool_dynamic.enqueue([it, L, PPR, al2_next, &instance_graph, time]
							...
							for (auto t : temp) {
								if (v < t) {
									...
									mtx_595[t].lock();
									auto query_result = graph_weighted_two_hop_extract_distance_and_hub_in_current_with_csv((*L)[t], Lv, t, v, shard);
									mtx_595[t].unlock();
									if (query_result.first > d1) {
										mtx_595[t].lock();
										insert_sorted_two_hop_label((*L)[t], v, d1, time, shard);
										mtx_595[t].unlock();
										mtx_595_1.lock();
								...
								if (t < v) {
									...
									if (query_result.first > d1) {
										mtx_595[v].lock();
										insert_sorted_two_hop_label((*L)[v], t, d1, time,shard);
										mtx_595[v].unlock();
										mtx_595_1.lock();
				...
				template <typename weight_type, typename hop_weight_type>
				inline void StrategyA2021NonHopIncrease<weight_type, hop_weight_type>::PI22(graph<weight_type> &instance_graph, std::vector<std::vector<two_hop_label<hop_weight_type>>> *L, PPR_TYPE::PPR_type *PPR, std::vector<pair_label> &al2_curr, std::vector<pair_label> *al2_next, ThreadPool &pool_dynamic, std::vector<std::future<int>> &results_dynamic, int time)
				{
						...
						results_dynamic.emplace_back(pool_dynamic.enqueue([it, L, PPR, al2_next, &instance_graph, time]
								...
								for (auto nei : instance_graph[it->first]) {
									if (nei.first > it->second) {
										mtx_595[it->first].lock();
										long long search_result = search_sorted_two_hop_label_in_current_with_csv((*L)[it->first], it->second,shard).first + (long long)nei.second;
										mtx_595[it->first].unlock();
										mtx_595[nei.first].lock();
										auto query_result = graph_weighted_two_hop_extract_distance_and_hub_in_current_with_csv((*L)[nei.first], Lxx, nei.first, it->second, shard);
										mtx_595[nei.first].unlock();
										if (query_result.first > search_result) {
											mtx_595[nei.first].lock();
											insert_sorted_two_hop_label((*L)[nei.first], it->second, search_result, time,shard);
											mtx_595[nei.first].unlock();
											mtx_595_1.lock();
