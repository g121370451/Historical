	...
	namespace nonhop
	{
		namespace algorithm2021
		{
			namespace decrease
			{
				...
				private:
					void ProDecreasep_batch(graph<weight_type> &instance_graph, std::vector<std::vector<two_hop_label<hop_weight_type>>> *L, PPR_TYPE::PPR_type *PPR,
											std::vector<affected_label> &CL_curr, std::vector<affected_label> *CL_next, ThreadPool &pool_dynamic, std::vector<std::future<int>> &results_dynamic, int time) const;
				};
				template <typename weight_type, typename hop_weight_type>
				inline void StrategyA2021NonHopDecrease<weight_type, hop_weight_type>::operator()(graph<weight_type> &instance_graph, two_hop_case_info<hop_weight_type> &mm, std::vector<std::pair<int, int>> &v, std::vector<weight_type> &w_new, ThreadPool &pool_dynamic, std::vector<std::future<int>> &results_dynamic, int time) const
				{
					...
					for (auto &it : w_new_map)
					{
						...
						for (int sl = 0; sl < 2; sl++)
						{
							...
							for (auto it : L[v1])
							{
								...
								if (v <= v2)
								{
									auto query_result = graph_weighted_two_hop_extract_distance_and_hub_in_current_with_csv(L[v], L[v2], v, v2, shard); // query_result is {distance, common hub}
									if (query_result.first > dis)
									{
										mtx_595[v2].lock();
										insert_sorted_two_hop_label(L[v2], v, dis, time, shard);
										mtx_595[v2].unlock();
									...
									else
									{
										...
										{
											mtx_595[v2].lock();
											insert_sorted_two_hop_label(L[v2], search_result.second, dis, time, shard);
											mtx_595[v2].unlock();
				...
				template <typename weight_type, typename hop_weight_type>
				inline void StrategyA2021NonHopDecrease<weight_type, hop_weight_type>::ProDecreasep_batch(graph<weight_type> &instance_graph, std::vector<std::vector<two_hop_label<hop_weight_type>>> *L, PPR_TYPE::PPR_type *PPR, std::vector<affected_label> &CL_curr, std::vector<affected_label> *CL_next, ThreadPool &pool_dynamic, std::vector<std::future<int>> &results_dynamic, int time) const
				{
					bool is_debug = false;
					...
					for (const affected_label &it : CL_curr)
					{
						results_dynamic.emplace_back(pool_dynamic.enqueue([time, it, L, PPR, CL_next, &instance_graph, is_debug]
								...
								for (auto nei : instance_graph[v]) {
									...
									if (u < vnei) {
										mtx_595[vnei].lock();
										auto query_result = graph_weighted_two_hop_extract_distance_and_hub_in_current_with_csv((*L)[vnei], Lu, vnei, u, shard); // query_result is {distance, common hub}
										mtx_595[vnei].unlock();
										if (query_result.first > dnew) {
											...
											insert_sorted_two_hop_label((*L)[vnei], u, dnew, time,shard);
											mtx_595[vnei].unlock();
											if (experiment::status::currentTimeMode == experiment::status::SLOT1)
											{
												++shard.label_decrease_insert_slot1;
											}
											else
											{
												++shard.label_decrease_insert_slot2;
											}
											mtx_595_1.lock();
											...
											mtx_595[vnei].lock();
											std::pair<int, int> search_result = search_sorted_two_hop_label_in_current_with_csv((*L)[vnei], u,shard);
											mtx_595[vnei].unlock();
											if (search_result.first < 1e7 && search_result.first > dnew) {
												...
												insert_sorted_two_hop_label((*L)[vnei], search_result.second, dnew, time, shard);
